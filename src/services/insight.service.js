...
import { predictBalance } from "./prediction.js";

export const generateInsights = async (userId) => {
  try {
    const user = await User.findByPk(userId);

    // 🔥 STEP 0: Get prediction (safe)
    let predictions = [];

    try {
      const result = await predictBalance(userId);
      predictions = result?.predictions || []; // 🔥 FIX
    } catch (err) {
      console.error("Prediction failed:", err.message);
    }

    const lowBalance = predictions.find((p) => p.balance < 2000);
    // =========================
    // ✅ FETCH TRANSACTIONS
    // =========================
    ...
    if (!transactions.length) {
      return {
        ...
        top_category: null,
        message:
          "No spending data available yet. Add transactions to get insights.",
      };
    }
    ...
    const topCategoryPercentage =
      total_spend > 0 ? Math.round((maxSpend / total_spend) * 100) : 0;

    // =========================
    // ✅ STEP 1: RULE-BASED MESSAGE
    // =========================
    let message;
    ...
    if (user?.monthly_income && total_spend > 0) {
      const incomePercent = Math.round(
        (total_spend / user.monthly_income) * 100,
      );
    ...
    // 🔥 NEW: Low balance warning (rule-based)
    if (lowBalance) {
      message += ` Your balance may drop below ₹2000 around ${lowBalance.date}.`;
    }
    ...
    const insightData = {
      ...
      // 🔥 NEW FIELDS (CORE FEATURE)
      current_balance: user?.current_balance || 0,
      low_balance_date: lowBalance?.date || null,
    };
    ...
    return {
      ...
      breakdown,
      top_category,
      current_balance: user?.current_balance || 0, // 🔥 added
      low_balance_date: lowBalance?.date || null, // 🔥 added
      message: aiMessage,
    };
  } catch (error) {
